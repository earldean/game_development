...
namespace ProgrammingAssignment4
{
    ...
    public class TeddyBear
    {
        ...
        public TeddyBear(Texture2D sprite, Vector2 location)
        {
			...
			drawRectangle.X = (int) this.location.X - halfDrawRectangleWidth;
			drawRectangle.Y = (int) this.location.Y - halfDrawRectangleHeight;

			Console.WriteLine (halfDrawRectangleWidth + " " + halfDrawRectangleHeight);
        }
        ...
        public void SetTarget(Vector2 target)
        {
			...
			int deltaX = (int) (target.X - location.X);
			int deltaY = (int) (target.Y - location.Y);
			...
			int magnitude = (int) Math.Sqrt (Math.Pow (deltaX, 2) + Math.Pow(deltaY, 2));

			// Normalize the velocity vector by dividing the components by the magnitude
